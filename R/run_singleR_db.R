...
=======
    ...
    so_in$HPCA_fine <- run_singleR(
      so_in, fetch_celldex_ref("hpca"), "label.fine"
    )
...
=======
    ...
    so_in$BP_encode_fine <- run_singleR(
      so_in, fetch_celldex_ref("BP_encode"), "label.fine"
    )
    so_in$BP_encode_ont <- run_singleR(
      so_in, fetch_celldex_ref("BP_encode"), "label.ont"
    )
    so_in$BP_encode_ont <- cell_ont$name[so_in$BP_encode_ont]

    so_in$monaco_main <- run_singleR(
      so_in, fetch_celldex_ref("monaco"), "label.main"
    )
    so_in$monaco_fine <- run_singleR(
      so_in, fetch_celldex_ref("monaco"), "label.fine"
    )
    so_in$monaco_ont <- run_singleR(
      so_in, fetch_celldex_ref("monaco"), "label.ont"
    )
    so_in$monaco_ont <- cell_ont$name[so_in$monaco_ont]

    so_in$immu_cell_exp_main <- run_singleR(
      so_in, fetch_celldex_ref("dice"), "label.main"
    )
    so_in$immu_cell_exp_fine <- run_singleR(
      so_in, fetch_celldex_ref("dice"), "label.fine"
    )
    so_in$immu_cell_exp_ont <- run_singleR(
...
>>>>>>> 6c9f3fa (feat: tests and plotting function updates)
    ...
    so_in$immgen_main <- run_singleR(
      so_in, fetch_celldex_ref("immgen"), "label.main"
    )
    so_in$immgen_fine <- run_singleR(
      so_in, fetch_celldex_ref("immgen"), "label.fine"
    )
    so_in$immgen_ont <- run_singleR(
      so_in, fetch_celldex_ref("immgen"), "label.ont"
    )
...
=======
    ...
    so_in$mouseRNAseq_fine <- run_singleR(
      so_in, fetch_celldex_ref("mouseRNAseq"), "label.fine"
    )
